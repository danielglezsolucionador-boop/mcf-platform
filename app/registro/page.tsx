...
function getPasswordStrength(password: string): { label: string; color: string; width: string } {
  ...
  if (password.length < 6) return { label: 'Muy corta', color: '#E63946', width: '20%' };
  if (password.length < 8) return { label: 'Débil', color: '#FB923C', width: '40%' };
  if (!/[A-Z]/.test(password) || !/[0-9]/.test(password)) return { label: 'Regular', color: '#FBBF24', width: '60%' };
  if (password.length >= 10 && /[!@#$%^&*]/.test(password)) return { label: 'Muy fuerte', color: '#22C55E', width: '100%' };
  return { label: 'Fuerte', color: '#4A90D9', width: '80%' };
}
...
export default function RegistroPage() {
  ...
  const validate = (): boolean => {
    ...
    if (!form.nombre.trim()) newErrors.nombre = 'Tu nombre es obligatorio';
    else if (form.nombre.trim().length < 3) newErrors.nombre = 'Ingresa tu nombre completo';
    ...
    if (!form.confirmarPassword) newErrors.confirmarPassword = 'Confirma tu contraseña';
    else if (form.password !== form.confirmarPassword) newErrors.confirmarPassword = 'Las contraseñas no coinciden';
  ...
  const handleGoogle = async () => {
    ...
    alert('Registro con Google (Demo)');
  };
        ...
        <AuthCard>
          <div className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-3">
              <button
                onClick={() => (window.location.href = '/registro/clave-sol')}
              ...
              >
                <SunatIcon size={20} />
                <span>Registrarse con Clave SOL</span>
            ...
            <Divider text="o completa tus datos" />
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              <label className="flex items-start gap-3 cursor-pointer select-none">
                ...
                <span className="text-xs text-gray-500 leading-relaxed">
                  Acepto los{' '}
                  <a href="#" className="font-semibold underline" style={{ color: '#1B3A6B' }}>
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-3.5 px-4 rounded-2xl font-bold text-white transition-all duration-200 hover:opacity-90 active:scale-95 shadow-md disabled:opacity-60 flex items-center justify-center gap-2 mt-1"
